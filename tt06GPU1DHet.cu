...
__global__ void compute_voltage(float* x, float* V, float* Iion, float step, float* randNums, int variations, int length, int width, int num_changing_vars, int time, float stimDur, float stimAmp, int tstim, float* s2time, int cells_per_thread, bool s2_analysis, int s2_loc) {
...
__global__ void make_randomNums(curandState *state, float* randArray) {
	int idx = blockDim.x*blockIdx.x + threadIdx.x ;
	float sigma = 0.15; //SD of variation of parameter
...
int main( int argc, const char* argv[] ) 
{
	...
	printf("Before creating Parameters\n");
	if (num_changing_vars != 0) {
		int seed_factor;
		...
		cudaMalloc( &dev_randNums, sizeof(float)*simulations*num_changing_vars);
		randNums = (float*)malloc(sizeof(float)*simulations*num_changing_vars);
	...
	*/
	...
	if (num_changing_vars != 0) { 
		...
		cudaMemcpy(randNums, dev_randNums, num_changing_vars*simulations*sizeof(float), cudaMemcpyDeviceToHost);	
		
	
		fprintf(params, "A = [ \n");
		for (i=0;i<simulations;i++) {
			for (ii=0;ii<num_changing_vars;ii++) {
				fprintf(params, "%f\t", randNums[(i*num_changing_vars)+ii]);
			}
			fprintf(params, "\n");
		}
		fprintf(params, "]; \n");
