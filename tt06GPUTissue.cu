...
__global__ void initialConditions(float* vars, int num_param, int num_cells, int* passed) {
	float V = -83.5092;
	float m = 0.0025;
	float h = 0.6945;
	float j = 0.6924;
...
__global__ void compute_voltage(float* x, float* V, float* Iion, float step, float* randNums, int variations, int length, int width, int num_changing_vars, int time, float stimDur, float stimAmp, int tstim, bool local, int* passed, int threshold) {
	...
	if (blockIdx.y == 0 && threadIdx.y == 0) { // Top Edge
		Vnet_U = 0.0;
	}
	else {
	...
	if (blockIdx.y == gridDim.y - 1 && threadIdx.y == blockDim.y - 1) { // Bottom Edge
		Vnet_D = 0.0;
	}
	else {
	...
	if (threadIdx.x == (blockDim.x - 1)) { // Right Edge
		Vnet_R = 0.0;
...
__global__ void make_randomNums( float* randArray, int num_cells, int num_changing_vars, int sf) {
	...
	float sigma2D = 0.15; //SD of variation of cells within cable
	int i;
...
int main(int argc, const char* argv[])
{
	...
	*/
	...
	printf("\n");
	fprintf(timer,"Elapsed Time = %f s \n", elapsedTime / 1000);
